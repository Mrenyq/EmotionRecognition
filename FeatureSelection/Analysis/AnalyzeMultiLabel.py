...
feature_selector = []
for f in features:
    if f.shape[1] > TH_FEATURES_NUM:
        n_features = TH_FEATURES_NUM
    else:
...
feature_selector = Parallel(n_jobs=-1)(
    [delayed(fitJMIM)(x, y, s, i) for i, (x, s) in enumerate(zip(features, feature_selector))])
feature_selector.sort(key=lambda x: x[1])
feature_selector = [s[0] for s in feature_selector]

# Save result
path_result = "G:\\usr\\nishihara\\data\\Yamaha-Experiment\\jmim_results_multilabel\\"
...
result_df_ranking = pd.DataFrame(results_ranking,
                                 index=["Ranking_EDA", "Ranking_PPG", "Ranking_Resp", "Ranking_ECG", "Ranking_ECG_Resp",
                                        "Ranking_EEG"]).T
result_df.to_csv(path_result + "jmim_feature_analysis_multi_" + str(STRIDE) + ".csv", index=False)
result_df_ranking.to_csv(path_result + "jmim_feature_ranking_multi_" + str(STRIDE) + ".csv", index=False)
