...
class EnsembleStudentOneDim(tf.keras.Model):
    ...
    def trainM(self, X, y_ar, y_val, y_ar_t, y_val_t, z_t, th, alpha, global_batch_size, training=False):
        ...
        final_loss_ar = tf.nn.compute_average_loss(
            self.cross_loss(y_ar, z_ar) + (alpha * self.cross_loss(y_ar_t, z_ar)), global_batch_size=global_batch_size)
        final_loss_val = tf.nn.compute_average_loss(
            self.cross_loss(y_val, z_val) + (alpha * self.cross_loss(y_val_t, z_val)),
            global_batch_size=global_batch_size)
...
class EnsembleStudentOneDim_MClass(tf.keras.Model):
    ...
    def trainM(self, X, y, y_t, z_t, T, alpha, global_batch_size, training=False):
        ...
        final_loss = tf.nn.compute_average_loss(
            self.sparse_cross_loss(y, logit) + (alpha * self.cross_loss(y_t, logit / T)),
            global_batch_size=global_batch_size)
